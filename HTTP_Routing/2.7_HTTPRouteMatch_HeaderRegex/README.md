...
```bash
curl --resolve tea.f5bnk.com:80:40.30.20.20 -H 'env: canary-01' http://tea.f5bnk.com/
```
...
네이티브 `headers.type: RegularExpression` 은 미적용. `gw-http-route_iRule.yaml` 은 `env` 헤더 Tcl `regexp` 후 `pool web-pool-httpbin-pool-pool` / `web-pool-tea-pool-pool`.  

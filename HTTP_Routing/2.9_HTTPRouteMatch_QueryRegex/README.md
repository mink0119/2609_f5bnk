...
네이티브 `queryParams.type: RegularExpression` 은 미적용. `gw-http-route_iRule.yaml` 은 `URI::query ver` + Tcl `regexp` 후 `pool web-pool-coffee-pool-pool` / `web-pool-tea-pool-pool`.  
